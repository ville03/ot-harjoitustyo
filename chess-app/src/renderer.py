...
class Renderer:
    ...
    def render(self):
        ...
        piece_font = pygame.font.SysFont(
            pygame.font.get_default_font(), square_side_length)

        for i in range(8):
            for j in range(8):
                square_center_location_height = (
                    height-(8-2*i)*square_side_length)//2
                square_center_location_width = (
                    width-(8-2*j)*square_side_length)//2
                ...
                text_surface = piece_font.render(
                    list(self._board[i][j])[1], False, color)
                piece_location = (square_center_location_width+square_side_length //
                                  4, square_center_location_height+square_side_length//4)
